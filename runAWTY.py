...
# Constants
JMTPATH = '~/JMT/JMT-singlejar-1.0.4.jar'
...
ATTRIBUTE_NAME = ['num_CL1', 'num_AP', 'AWTY_interval', 'S_d1', 'S_dAP', 'S_s1', 'mean', 'lower', 'upper']
SERVICE_DEVICE_CL1 = [0.02]
SERVICE_DEVICE_AP = [0.01]
SERVICE_SERVER_CL1 = [0.04]
SERVICE_SERVER_AP = 0
WAIT_INTERVAL = [0.00001] + list(np.arange(0.01, 0.4, 0.01))
...
rate_device_cl1 = [1/s for s in SERVICE_DEVICE_CL1]
rate_device_AP = [1/s for s in SERVICE_DEVICE_AP]
rate_server_cl1 = [1/s for s in SERVICE_SERVER_CL1]
rate_wait = [1/s for s in WAIT_INTERVAL]
		...
		for r_dAP in rate_device_AP:
			for r_d1 in rate_device_cl1:
				for r_s1 in rate_server_cl1:
					...
					collectResults(resultname, [x for x in params[0:2]] + [1/x for x in params[2:len(params)]])
